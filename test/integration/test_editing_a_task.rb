...
class EditingATaskTest < Minitest::Test
  ...
  def test_happy_path_edit_task_name
    ...
    IO.popen('./maintenance_tracker', 'r+') do |pipe|
      ...
      pipe.puts "1"
      expected_output << "What task would you like to edit? (or type exit to leave)\n"
      expected_output << "1. Oil Change at 10000 miles\n"
      expected_output << "2. Rotato Tires at 20000 miles\n"
      pipe.puts "2"
      expected_output << "What would you like to edit: name, mileage, or completion\n"
      pipe.puts "name"
  ...
  def test_happy_path_edit_task_completion
    shell_output = ""
    expected_output = main_menu
    create_car(2000, "VW", "Jetta")
    car_id = Database.execute("SELECT last_insert_rowid()")[0]['last_insert_rowid()']
    ...
    IO.popen('./maintenance_tracker', 'r+') do |pipe|
      ...
      pipe.puts "2"
      expected_output << "What would you like to edit: name, mileage, or completion\n"
      pipe.puts "completion"
      expected_output << "Mark task as completed? (y/n)\n"
      pipe.puts "210000"
      expected_output << "\n\nRotato Tires marked as completed\n\n"
      expected_output << main_menu
      pipe.puts "11"
      shell_output = pipe.read
      pipe.close_read
